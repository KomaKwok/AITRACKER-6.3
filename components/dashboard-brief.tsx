...
import { DashboardBrief, Signal } from "@/lib/types";
import { formatRelativeDate } from "@/lib/utils";

export function DashboardBriefCard({ brief, locale, signals }: { brief?: DashboardBrief; locale: "zh" | "en"; signals: Signal[] }) {
  const evidence = (brief?.sourceSignalIds ?? []).flatMap((id) => signals.filter((signal) => signal.id === id));
  const headline =
  ...
  return (
    <div className="mt-6 overflow-hidden rounded-[1.9rem] border border-violet-200/70 bg-[radial-gradient(circle_at_top_right,rgba(139,92,246,0.16),transparent_38%),linear-gradient(135deg,#faf7ff_0%,#f3f7ff_100%)] p-5">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0 max-w-3xl">
          <div className="inline-flex items-center gap-2 rounded-full bg-violet-100 px-3 py-1 text-xs font-semibold text-violet-700">
            ...
            {locale === "zh" ? "近期事实摘要" : "RECENT SOURCE BRIEF"}
          </div>
      ...
      {evidence.length > 0 && <div className="mt-4 flex flex-wrap gap-3 text-xs text-violet-700">
        <span>{locale === "zh" ? "依据原文：" : "Sources:"}</span>
        {evidence.map((signal) => <a key={signal.id} href={signal.url} target="_blank" rel="noreferrer" className="underline">
          {signal.publishedAt.slice(0, 10)} · {locale === "zh" ? signal.titleZh || signal.title : signal.title}
        </a>)}
      </div>}
      {bullets?.length ? (
